...
Почему выбран std::deque, а не std::stack?

    Ограничение задания:
        Если в следующем упражнении будет использоваться std::stack, то он запрещен в текущем. Поэтому был выбран std::deque, который может работать как стек, но не накладывает ограничений на доступ к другим элементам.
